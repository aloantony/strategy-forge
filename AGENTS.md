...
- **backend/strategy_builder/indicators.py** — Single source of truth for the 13 indicators (params, column templates + Spanish labels, codegen compute lines/helpers, Data Window fields, overlay roles); drives the generators and `/api/builder/meta`
...
- **backend/strategy_builder/mtf_generator.py** — MTF (schema v2) generator: any registry indicator per timeframe; `strategy_type` "rules" (root buy/sell condition trees with `TF.column` refs → simple signals) or "blocks" (risk tiers, ATR SL/TP, pyramiding, `direction` long|short)
