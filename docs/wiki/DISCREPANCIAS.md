...
| 3 | Comparación de backtest | Clave de resultados ambigua | La comparación devuelve la clave **`strategies`** | `backend/backtesting/runtime.py:806-814` |
| 4 | Costes en el CLI | `spread/slippage/commission` listados como argumentos del CLI | Existen en `BacktestRequest` pero el CLI **no** los expone | `backend/backtesting/cli.py:79-104` |
| 5 | `--warmup-bars` | "≥ 500" sin más | Default del CLI = `0`; el motor aplica `max(BARS_HISTORY, 500)` = 500 | `backend/backtesting/cli.py:95`, `backend/backtesting/runtime.py:57-58` |
...
| 7 | Tipos de acción del runtime v1 | Lista parcial | 9 tipos reales en `SUPPORTED_ACTION_TYPES` | `backend/runtime/plan_interpreter.py:189-199` |
| 8 | Repos del `UnitOfWork` | Lista aproximada | 11 repos reales (instances, symbol_books, strategy_state, plans, execution_reports, action_reports, entry_groups, legs, fills, pending_orders, event_log) | `backend/persistence/dal.py:733-743` |
| 9 | `decode_trade_comment` | Atribuido a `comment.py` | Vive en `backend/brokers/mt5/trading.py` (que re-exporta `build_trade_comment` de `comment.py`) | `trading.py:23-121` |
...
| 11 | Fábrica de datos | Implicaba MT5/Dukascopy/File en la fábrica | `build_data_source` solo crea `mt5`/`dukascopy`; `FileProvider` existe pero no está cableado | `backend/data/factory.py:88-152` |
| 12 | `scripts/debug.py` | No documentado | Runner de depuración (`datasource`, `backtest`, `factory`) | `scripts/debug.py` |
| 13 | Ticker Dukascopy de GER40 | Genérico | Canónico `GER40` → Dukascopy `E_DAAX`, MT5 `#Germany40` | `backend/data/symbols.json` |
| 14 | Esquema de persistencia | Tablas aproximadas | 12 migraciones reales; PRAGMAs WAL/FK/FULL | `backend/persistence/schema.py` |
...
- Se extrajeron **`configuracion.html`** (referencia de `backend/core/config.py`) y **`strategy-builder.html`**
