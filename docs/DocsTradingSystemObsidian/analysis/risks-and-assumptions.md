...
| Divergencia live/backtest | Resultados historicos no replican live. | Mantener `strategy_runtime.py` como capa comun y documentar diferencias. |
| Cambios inseguros en `gui_charts.py` | Rotura silenciosa de JS embebido o threading. | Usar Grace para specs y Felix para implementacion GUI. |
| Proceso de negocio enterrado en GUI | Difícil de probar y corregir por agentes backend. | Extraer validacion/orquestacion a `src/application/`. |
...
- El simbolo por defecto es `#Germany40`, configurable en [config.py](../../../config.py).
